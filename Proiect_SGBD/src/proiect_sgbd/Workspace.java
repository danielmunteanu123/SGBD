    ...
    private int insertNewPKValToNewFKColSingle() {
        for (int t = 0; t < refA.size(); t++) {
            ...
            if (!flexible.contains(currentRow[0])) {
                ...
                try {
                    query = "DECLARE CURSOR my_cursor IS SELECT " + pk_column_name + "," + new_pk_column + " FROM " + pk_tab + "; my_rec my_cursor%ROWTYPE; BEGIN FOR my_rec IN my_cursor LOOP UPDATE " + fk_tab + " SET " + new_fk_column + "=my_rec." + new_pk_column + " WHERE " + col + "=" + "my_rec." + pk_column_name + "; END LOOP; END;";
                    st = conn.createStatement();
                    rs = st.executeQuery(query);
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(null, e.getMessage(), "Eroare", JOptionPane.ERROR_MESSAGE);
